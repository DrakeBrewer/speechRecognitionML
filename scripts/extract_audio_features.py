...
SAMPLE_RATE = 22050
WINDOW_LEN = 1.0   			                                                            # 1 sec clip length
HOP_LEN = 0.5      			                                                            # overlap 50%
SILENCE_THRESHOLD = 0.01  			                                                    # skip quiet sections

def extract_features(file_path, speaker):

    # define librosa feature functions
    ...
    for i in range(frames.shape[1]):
        ...
        mfcc = mfcc_feat(y=window, sr=sampling_rate, n_mfcc=13)
        ...
        for idx, feat in enumerate(feature_list):
            if idx == 3 or idx == 4:
                ...
                features.extend(np.mean(feat, axis=1))                  # 2D -> 1D

        features.append(speaker)                                        # add speaker name
...
def main():
    ...
    for file in AUDIO_DIR.glob("*.wav"):
        ...
        df = extract_features(file, file.stem)
        data_frames.append(df)

    # combine all df's in one array
